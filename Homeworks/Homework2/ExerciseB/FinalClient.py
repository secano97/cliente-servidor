...
def threadOfSend(dest):
	...
	while True:
		...
		socket_SEND.send_multipart([bytes("send",'ascii'),bytes(dest,'ascii'),data])
		msg = socket_SEND.recv_json()
		if msg ["respuesta"] == "lol":
			print(" lol")
		else:
			print("Error! in lol")
			print(msg["respuesta"])
...
def menu():
	...
	socket_SEND.send_multipart([bytes("to",'ascii'),bytes(dest,'ascii')])
	...
	hilo1 = threading.Thread(target=threadOfSend (dest, ))
	#print("hilo1")
